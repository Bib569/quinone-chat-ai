...
export function getDatasetSummary(): string {
  const families = ['benzoquinone', 'naphthoquinone', 'anthraquinone'];
  const summary = families.map((f) => {
    const entries = QUINONE_DATASET.filter((e) => e.family === f);
    const gaps = entries.map((e) => e.homo_lumo_gap_ev);
    return `${f} (${entries.length}): gap ${Math.min(...gaps).toFixed(2)}–${Math.max(...gaps).toFixed(2)} eV`;
  });
  return `Dataset: 25 quinone derivatives\n${summary.join('\n')}`;
}
